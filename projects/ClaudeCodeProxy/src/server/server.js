...
// Usage tracker for proxied responses (streamed)
function recordUsageFromBody(req, data) {
  try {
    ...
    try {
      if (req.body && typeof req.body === 'object') {
        modelName = req.body.model || DEFAULT_MODEL;
      }
    } catch (e) {
    ...
    if (req.user) {
      ...
      if (!req.user.isAdmin && req.user.usage.balance !== undefined) {
...
const anthropicProxy = createProxyMiddleware({
  ...
  onProxyRes: (proxyRes, req, res) => {
    // Buffer response body to record usage once the stream ends
    const chunks = [];
    ...
    proxyRes.on('end', () => {
      ...
      if (bodyStr) {
        recordUsageFromBody(req, bodyStr);
      }
    });
  }
});
...
app.get('/admin/usage', authenticate, requireAdmin, (req, res) => {
  ...
  try {
    ...
    for (const file of logFiles) {
      ...
      const fileDate = file.split('_')[1] && file.split('_')[1].split('.')[0];
