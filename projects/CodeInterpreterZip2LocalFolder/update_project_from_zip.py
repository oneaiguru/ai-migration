    ...
    try:
        with ZipFile(zip_path, "r") as zip_ref:
            ...
            for member in zip_ref.namelist():
                # Skip if path contains __MACOSX or __pycache__
                if '__MACOSX/' in member or '__pycache__/' in member:
                    continue
                    
                # Skip macOS metadata files (starting with ._) at any level
                parts = Path(member).parts
                if any(part.startswith('._') for part in parts):
                    continue
                    
                # Skip __pycache__ directories at any level
                if any(part.startswith('__pycache__') for part in parts):
                    continue

                member_path = (target_dir / member).resolve()
                if not str(member_path).startswith(str(target_root)):
                    logger.warning(f"Skipping unsafe member outside target: {member}")
                    continue

                valid_members.append((member, member_path))
            
            # First create all necessary directories
            for member, member_path in valid_members:
            ...
            for member, member_path in valid_members:
                if not member.endswith('/'):  # Only process files
