...
class DataExporter {
    ...
    private function applyFilters($item, $filters) {
        ...
        foreach ($itemFilters as $field => $value) {
            ...
            if ($value === null) {
                ...
                if (isset($value['operator']) && isset($value['value'])) {
                    if (!$this->applyOperator($item[$field], $value['operator'], $value['value'])) {
                        return false;
                    }
                }
                // Array of operator/value clauses (AND all)
                else if ($this->isOperatorList($value)) {
                    foreach ($value as $clause) {
                        if (!$this->applyOperator($item[$field], $clause['operator'], $clause['value'])) {
                            return false;
                        }
                    }
                }
    ...
    /**
     * Apply a single operator clause to a value.
     */
    private function applyOperator($fieldValue, $operator, $expected) {
        switch ($operator) {
            case '=':
    ...
    /**
     * Determine if the value is a list of operator/value clauses.
     */
    private function isOperatorList($value) {
        if (!is_array($value) || empty($value)) {
            return false;
        }
        foreach ($value as $clause) {
            if (!is_array($clause) || !isset($clause['operator'], $clause['value'])) {
                return false;
            }
        }
        return true;
