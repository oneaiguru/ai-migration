...
* **Efficiency (E):** `successful_features / capacity_unit`
  * Codex/Claude: capacity unit = Δbar percentage points (**pp**) over window
  * GLM: capacity unit = prompts used
  * Note: "pp" = percentage points (absolute %, not relative change)
...
* **Claude `/usage`** — session/weekly bar (single pool; Opus shares the weekly meter); thinking level
...
| **Claude Code (Anthropic)** | Pro / Max ×5 / Max ×20 | 5h + weekly (single pool) | Via `/usage` bars | Sonnet 4.5, Opus 4.1 | Thinking Levels: think/ultrathink |
...
**Tags must appear at START of commit subject line for reliable grepping.**
...
* Lowercase, anchored at line start = deterministic grep
* Example: `git log --grep="^xfeat::" --not --grep="^xproto::"`
...
### A) JSONL Schemas
See §2; keys: `meters.{fiveh_pct,weekly_pct,...}`; `providers.delta_weekly_pp`, `glm.prompts_used`
...
* **v1.6-final:** Simplified tags (`xfeat::`/`xproto::`), explicit review panel with capacity guardrail, terminology split (Reasoning vs Thinking), 10pp buffer rationale, GLM 2× guidance, pp definition, commit format clarification
