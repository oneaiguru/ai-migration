...
```mermaid
graph TD
    User[User] -->|Commands| TelegramBot[Telegram Bot Service]
    TelegramBot -->|State Management| Redis[Redis Cache]
    TelegramBot -->|Story Processing| StoryEngine[Story Engine]
    StoryEngine -->|Data Storage| PostgreSQL[PostgreSQL Database]
    TelegramBot -->|AI Integration| OpenAI[OpenAI Service]
    TelegramBot -->|Payment Processing| YooMoney[YooMoney Integration]
    TelegramBot -->|Error Reporting| ErrorTracking[Error Tracking Service]
    TelegramBot -->|Metrics| Prometheus[Prometheus Metrics]
```
...
### 1.3 Telegram Bot Specific Implementation
