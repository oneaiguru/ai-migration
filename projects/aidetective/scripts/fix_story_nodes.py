...
# Allow running without Telegram credentials (used only for DB fix)
os.environ.setdefault("ALLOW_MISSING_TELEGRAM_TOKEN", "true")

# Add root directory to path to access our modules
sys.path.append('.')
...
async def fix_story_nodes():
    ...
    try:
        ...
        if not db_url:
            db_url = "sqlite+aiosqlite:///app/data/sherlock.db"  # Fallback aligned with runtime
            logger.warning(f"Using fallback database URL: {db_url}")
        else:
            logger.info(f"Using database URL from settings: {db_url}")
