...
function getStats(array $config): array {
    ...
    if (isset($reportData['stats']) && $reportData['stats']['unique'] > 0) {
        ...
        // В реальной системе нужно адаптировать расчет под фактическую структуру данных
...
function startProcess(array $data, array $config): array {
    ...
    exec(sprintf('%s -v > /dev/null 2>&1', escapeshellcmd($phpBinary)), $phpOutput, $phpStatus);
    if ($phpStatus !== 0) {
        throw new Exception('PHP не найден или недоступен для запуска фонового процесса');
    }
...
function testProduct(array $data, array $config): array {
    ...
    $generatedText = $claudeClient->generateText($prompt);
...
function getSystemStatus(array $config): array {
    ...
    // Проверка подключения к Bitrix API
    try {
        $bitrixClient = new BitrixApiClient(
            $config['bitrix']['endpoint'],
            $config['bitrix']['login'],
    ...
    try {
        if (!empty($config['claude']['api_key']) && 
            ...
            // Недорогой пинг вместо платной генерации
