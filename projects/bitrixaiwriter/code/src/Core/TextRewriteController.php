...
class TextRewriteController {
    ...
    private function processBatch(array $batch): void {
        ...
        foreach ($batch as $product) {
            ...
            try {
                ...
                $pendingTexts[$productId] = $newText;
            ...
            try {
                ...
                foreach ($results as $productId => $result) {
                    if ($result['success']) {
                        ...
                        if ($this->config['save_history'] && isset($pendingTexts[$productId])) {
                            $this->textDatabase[] = $pendingTexts[$productId];
                            $this->uniquenessChecker->addTextsForComparison([$pendingTexts[$productId]]);
                        }
                    } else {
    ...
    private function generateUniqueText(int $productId, string $productName, string $originalText) {
        ...
        while ($attempts < $maxAttempts) {
            try {
                ...
                // Не перезаписываем описание пустым ответом в случае сбоя генерации
    ...
    private function resolveOutputDir(string $outputDir): string {
        ...
        // Get project root (assuming this file is in code/src/Core/)
...
class SimpleLogger {
    ...
    public function __construct(string $logFile) {
        ...
        // Create log directory when a path like logs/rewrite.log is provided
