<?php
/**
 * Командный интерфейс для запуска системы автоматизации переписывания текстов
 * 
 * Позволяет запускать различные операции системы через командную строку
 * 
 * Использование:
 *   php cli.php --action=<action> [options]
 * 
 * Доступные действия:
...
// Автозагрузка классов (поддержка запуска из корня и из code/)
...
if (!$autoloaderLoaded) {
    fwrite(STDERR, "Warning: composer autoloader not found; continuing with manual requires\n");
}

// Явно подключаем внутренние классы, так как они без пространств имен и не подхватываются composer'ом
require_once __DIR__ . '/Api/BitrixApiClient.php';
require_once __DIR__ . '/Api/ClaudeApiClient.php';
require_once __DIR__ . '/Core/PromptGenerator.php';
require_once __DIR__ . '/Core/UniquenessChecker.php';
require_once __DIR__ . '/Core/TextRewriteController.php';

// Загрузка конфигурации (ищем в code/config, затем рядом со скриптом для обратной совместимости)
$configCandidates = [
...
if ($configFile === null) {
    die("Ошибка: Файл конфигурации не найден (искали: " . implode(', ', $configCandidates) . ")\n");
}
...
$options = getopt('', ['action::', 'filter::', 'limit::', 'product-id::', 'verbose::', 'config::', 'help::']);
...
try {
    ...
    switch ($action) {
        ...
        case 'test':
            testGeneration($controller, $claudeClient, $promptGenerator, $uniquenessChecker, $bitrixClient, $config, $options, $verbose);
            break;
...
function showHelp(): void {
    echo <<<EOT
Система автоматизации переписывания текстов
...
Примеры:
  php cli.php --action=process --limit=50
