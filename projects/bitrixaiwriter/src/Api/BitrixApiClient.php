<?php
/**
 * Класс для работы с API Битрикс (Мок версия)
 * 
 * Эмулирует работу с API Битрикс для тестирования
 * без необходимости реального подключения
 */
namespace Api;
...
class BitrixApiClient {
    ...
    /**
     * Эмулирует массовое обновление товаров
     *
    ...
    /**
     * Эмулирует экспорт товаров в CSV файл
     *
     * @param array $filter Фильтр для выборки товаров
    ...
    /**
     * Эмулирует импорт товаров из CSV файла
     *
    ...
    public function importProductsFromCsv(string $filePath, array $mapping, string $delimiter = ';'): array {
        ...
        while (($row = fgetcsv($file, 0, $delimiter)) !== false) {
            ...
            if ($headerCount !== $rowCount) {
                $results['failed']++;
                $results['errors'][] = "Row {$results['total']}: Header/row column count mismatch ({$headerCount} vs {$rowCount})";
                continue;
            }
