<?php
/**
 * Оптимизированный класс для проверки уникальности текста
 * 
 ...
 * С уменьшенным временем ожидания для более быстрой обработки транзакций
 */
namespace Core;
...
class UniquenessChecker {
     ...
     * Оптимизированная версия с уменьшенным временем ожидания (1 мс вместо 5 сек)
     *
    ...
    public function checkWithExternalApi(string $text): array {
        ...
        // Ожидаем результатов проверки (с пробами через каждую 1 миллисекунду вместо 5 секунд)
        $attempts = 0;
        // Даем сервису время собрать результат: до ~30 секунд при проверке каждые 1 секунду
        $maxAttempts = 30;
        $pollIntervalMicroseconds = 1000000;
        $uniqueness = null;
     ...
     * Оптимизированная версия с более быстрым алгоритмом
     * 
     * @param string $text Текст для проверки
    ...
    public function checkLocalUniqueness(string $text): array {
        ...
        // Оптимизация: создаем хэш-таблицу слов текста для быстрого поиска
        $textWordsHash = array_flip($textWords);
        
        foreach ($this->previousTexts as $index => $previousText) {
            $previousWords = $this->getWords($previousText);
            $similarity = $this->calculateOptimizedSimilarity($textWordsHash, $previousWords);
     ...
     * Оптимизированная версия для более быстрой обработки
     *
    ...
    /**
     * Оптимизированный метод вычисления сходства для более быстрой обработки
     *
     * @param array $textWordsHash Хэш-таблица слов первого текста
     * @param array $words2 Слова второго текста
     * @return float Коэффициент сходства (0-1)
     */
    private function calculateOptimizedSimilarity(array $textWordsHash, array $words2): float {
        // Создаем множество из слов второго текста
        $set2 = array_flip($words2);
        
        // Находим пересечение множеств (оптимизировано, так как textWordsHash уже готов)
        $intersection = array_intersect_key($textWordsHash, $set2);
        
        // Находим объединение множеств
        $union = $textWordsHash + $set2;
