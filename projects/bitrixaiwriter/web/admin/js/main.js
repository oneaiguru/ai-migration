...
function ensureApiKey() {
    if (ADMIN_API_KEY !== null && ADMIN_API_KEY !== undefined && ADMIN_API_KEY !== '') {
        return;
    }
    const input = prompt('Введите административный API ключ (оставьте пустым, если не настроен):', '');
    if (input !== null) {
        const trimmedInput = input.trim();
        ADMIN_API_KEY = trimmedInput || null;
        if (trimmedInput) {
 ...
 */
async function apiRequest(endpoint, method = 'GET', data = null, allowRetry = true, showErrorOnFailure = true) {
    try {
        ...
        if (!response.ok) {
            if (response.status === 401 && allowRetry) {
                ...
                if (ADMIN_API_KEY && ADMIN_API_KEY !== apiKey) {
                    return await apiRequest(endpoint, method, data, false, false);
                }
            }
    ...
    try {
        ...
        reportsData.reports.forEach(report => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-primary view-report" data-report="${report.filename}">Просмотр</button>
                    <button class="btn btn-sm btn-outline-danger delete-report" data-report="${report.filename}">Удалить</button>
                </td>
            `;
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        <tr><th>Всего обработано</th><td>${reportData.stats.total}</td></tr>
                        <tr><th>Успешно</th><td>${reportData.stats.success}</td></tr>
                        <tr><th>Неудачно</th><td>${reportData.stats.failed}</td></tr>
                        <tr><th>Уникальных</th><td>${reportData.stats.unique}</td></tr>
                        <tr><th>Недостаточно уникальных</th><td>${reportData.stats.not_unique}</td></tr>
                        <tr><th>Пропущено</th><td>${reportData.stats.skipped}</td></tr>
                    </table>
                </div>
            </div>
        `;
        ...
        if (reportData.stats.errors && reportData.stats.errors.length > 0) {
            content += `
                <div class="card">
                    ...
                    <div class="card-body">
                        <ul class="list-group">
                            ${reportData.stats.errors.map(error => `<li class="list-group-item text-danger">${error}</li>`).join('')}
                        </ul>
                    </div>
                </div>
            `;
