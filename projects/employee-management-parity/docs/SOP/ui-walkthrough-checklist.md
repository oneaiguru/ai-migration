...
Use this script to validate the parity demo in a browser session or with Playwright. Manual references: CH3 Employees (`${MANUALS_ROOT}/estimation/processing_manual/process/chapters/CH3_Employees.md`), CH5 Schedule chapters, screenshots noted in `docs/SCREENSHOT_INDEX.md`.
...
Log discrepancies in `docs/SESSION_HANDOFF.md` and note follow-ups directly in the relevant task spec under `docs/Tasks/` (add status notes or TODO bullets). Use the archived backlog only when updating Phase 1–5 history (e.g., `docs/Archive/Tasks/04_phase-4-accessibility-and-final-parity.md`).
