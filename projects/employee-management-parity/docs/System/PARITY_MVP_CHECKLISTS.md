...
- KPI grid – 6 карт, тренд-бейджи, прогресс (present, ${ANALYTICS_REPO}/src/features/analytics/KpiOverview.tsx:15)
- Line chart – 24h звонки с целевым уровнем (present, ${ANALYTICS_REPO}/src/features/analytics/KpiOverview.tsx:18)
- Report table – Матрица отделов (present, ${ANALYTICS_REPO}/src/features/analytics/KpiOverview.tsx:26)
- Gauges – SL / Util / CSAT полукольца (present, ${ANALYTICS_REPO}/src/features/analytics/AdvancedAnalytics.tsx:16)
- Heatmap – Нагрузка по дням/часам (present, ${ANALYTICS_REPO}/src/features/analytics/AdvancedAnalytics.tsx:29)
- Radar – Текущие vs целевые показатели (present, ${ANALYTICS_REPO}/src/features/analytics/AdvancedAnalytics.tsx:37)
- Trend analysis (dual-axis) – Present (primary/secondary оси, `${ANALYTICS_REPO}/src/features/analytics/AdvancedAnalytics.tsx:51`)
- Trend toolbar – Present (сглаживание/± цели, `${ANALYTICS_REPO}/src/features/analytics/AdvancedAnalytics.tsx:69`)
- Absenteeism snapshot – Present (`${ANALYTICS_REPO}/src/features/analytics/AbsenteeismPanel.tsx:10`)
- Forecast Builder – Present (`${ANALYTICS_REPO}/src/features/forecasting/ForecastBuilder.tsx:1` via shared module `@wfm/shared/forecasting`)
...
- Выполнить UAT пакеты (`parity_static`, `chart_visual_spec`) + сохранить скриншоты.
- Добавить Playwright/Storybook покрытия для новых обёрток.
- **Статус 2025-10-27:** UAT (`parity_static`, `chart_visual_spec`) — Pass (включая прогноз/отчёты). Новые скриншоты: `trend-analysis.png`, `forecast-build.png`, `reports-card.png`. См. `uat-agent-tasks/2025-10-27_consolidated-uat-sweep.md` (analytics rows) и обновлённый Findings файл.
...
- Requests table — `ReportTable` с RU форматированием и историей (present, ${EMPLOYEE_PORTAL_REPO}/src/pages/VacationRequests.tsx:183-597)
- Period history dialog — `Dialog` режим "За период" (фильтры по датам/статусам, счётчики) (present, `${EMPLOYEE_PORTAL_REPO}/src/pages/VacationRequests.tsx:926-1163`)
- New request dialog — `Dialog` + `FormField` (валидации, экстренная отметка, тосты) + `DateField` placeholder `дд.мм.гггг` (`${EMPLOYEE_PORTAL_REPO}/src/pages/VacationRequests.tsx:334-804`, `${EMPLOYEE_PORTAL_REPO}/src/components/inputs/DateField.tsx:1-134`)
- Profile edit form — `FormField` для персональных/контактных/рабочих/экстренных блоков (present, `${EMPLOYEE_PORTAL_REPO}/src/pages/Profile.tsx:214-732`)
- Locale helpers & RU copy — `${EMPLOYEE_PORTAL_REPO}/src/utils/format.ts:1-70`, `${EMPLOYEE_PORTAL_REPO}/src/locale/ru.ts:1-26`
- Статус UAT (2025-11-03) — **Re-run pending**: localisation fixes deployed to https://wfm-employee-portal-8h7oh0j1z-granins-projects.vercel.app; execute parity_static + trimmed_smoke, attach screenshots, and update `docs/Workspace/Coordinator/employee-portal/UAT_Findings_2025-10-13_template.md` with results.

### MVP Scope for Employee Portal
- Везде использовать RU локаль для дат/чисел (Dashboard, заявки, профиль).
- Провалидировать Новый отпуск (тип/дату начала/дату окончания, превью длительности, экстренность) и Профиль (обязательные поля, email/телефон, экстренный контакт), без HTML5 блокировок.
- Сохранить поведение parity-only: таблицы/диалоги/фильтры совпадают с эталоном, без визуальных правок.
- После деплоя прогнать `parity_static` и `trimmed_smoke`, зафиксировать URL и результаты в `docs/SESSION_HANDOFF.md`.
...
- Accuracy performance trend — `LineChart` (implemented, ${FORECASTING_ANALYTICS_REPO}/src/components/forecasting/accuracy/PerformanceChart.tsx:3). Dual-axis legend validated in smoke + UAT.
- Error distribution views (time/day/magnitude) — `BarChart` (implemented, ${FORECASTING_ANALYTICS_REPO}/src/components/forecasting/accuracy/ErrorAnalysis.tsx:9). Buttons confirmed during chart_visual_spec rerun.
- Trend dashboard — dual-axis `LineChart` + anomaly markers (implemented, ${FORECASTING_ANALYTICS_REPO}/src/components/forecasting/trends/TrendAnalysisDashboard.tsx:33). UAT 2025-10-27 Pass (`playwright-forecasting-trend.png`).
- Seasonality bars — `BarChart` with positive/negative stacks (same file lines 118-155). Renders successfully; palette follow-up remains.
- Manual adjustments grid — `ReportTable` + undo/redo panel (implemented, ${FORECASTING_ANALYTICS_REPO}/src/components/forecasting/ManualAdjustmentSystem.tsx:1). UAT 2025-10-27 Pass; awaiting real API wiring.

### MVP Scope for Forecasting & Analytics
- Confirm dual-axis trend renders target + anomaly badges; verify confidence band shading in live UAT. ✅ 2025-10-29 (React 18.3.1 lock)
- Validate error analysis toggles (time/day/magnitude) against UAT pack `chart_visual_spec`. ✅ 2025-10-29
- Manual adjustments now call API wrapper; connect to real backend validation/persist pipeline before sign-off. (Open)
- Routing fix + SPA rewrite deployed 2025-10-27; keep smoke script + npm ci requirement in release checklist.
