...
import type { BarChartProps, TargetLine } from './types';
import {
  formatTooltipValue,
...
const buildTargetDataset = (
  target: TargetLine,
  labels: string[],
): ChartDataset<'bar' | 'line', (number | ScatterDataPoint | null)[]> => {
  const label = sanitizeDatasetLabel(target.label);
  const values = target.series ?? labels.map(() => target.value ?? null);
  const dataset: ChartDataset<'bar' | 'line', (number | ScatterDataPoint | null)[]> = {
    type: 'line',
    label,
...
const BarChart: React.FC<BarChartProps> = ({
  ...
  const chartData = useMemo<
    ChartData<'bar' | 'line', (number | ScatterDataPoint | null)[], unknown>
  >(() => {
    const datasets: ChartDataset<'bar' | 'line', (number | ScatterDataPoint | null)[]>[] = activeSeries.map(
      (entry, idx) => {
        ...
        entry.points.forEach((point, index) => {
          const key = point.label ?? resolvedLabels[index] ?? String(index);
          valueMap.set(key, Number(point.value));
        });

        const dataset: ChartDataset<'bar' | 'line', (number | ScatterDataPoint | null)[]> = {
          type: 'bar',
          label,
          data: resolvedLabels.map((labelKey) => valueMap.get(labelKey) ?? null),
    ...
    return {
      ...
      datasets: datasets.concat(targets.map((target) => buildTargetDataset(target, resolvedLabels))),
    };
  }, [activeSeries, resolvedLabels, stacked, targets]);
