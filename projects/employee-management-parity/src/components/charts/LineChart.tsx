...
const buildTargetDataset = (
  ...
  const data: Array<number | ScatterDataPoint | null> = xValues.map((x, idx) => {
    const pointValue = values?.[idx] ?? target.value;
    if (pointValue == null) {
      if (useTimeScale) {
        const parsed = Date.parse(x);
        return { x: Number.isFinite(parsed) ? parsed : idx, y: Number.NaN };
      }
      return Number.NaN;
    }
    ...
    if (useTimeScale) {
      const parsed = Date.parse(x);
      return { x: Number.isFinite(parsed) ? parsed : idx, y: value };
    }
    return value;
  });

  const dataset: ChartDataset<'line', (number | ScatterDataPoint | null)[]> = {
    type: 'line' as const,
    label,
    data,
...
const data = useMemo<ChartData<'line', (number | ScatterDataPoint | null)[], unknown>>(() => {
    const baseCategories = !useTimeScale ? (labels.length ? labels : xValues) : undefined;

    const datasets: ChartDataset<'line', (number | ScatterDataPoint | null)[]>[] = series.map((entry, idx) => {
      ...
      const datasetData: Array<number | ScatterDataPoint | null> = useTimeScale
        ? points.map((point, pointIndex) => {
            const rawTimestamp = point.timestamp ?? points[pointIndex]?.timestamp ?? point.label ?? '';
            const parsed = typeof rawTimestamp === 'string' ? Date.parse(rawTimestamp) : Number(rawTimestamp);
            const xValue = Number.isFinite(parsed) ? parsed : pointIndex;
