<!-- ARCHIVED FILE -->
# 00 - Parity Backlog and Plan (archived)
...
15. **Phase 6 – Migration Planning**
    ...
    - Prep references: `ai-docs/README.md`, `MANIFEST.md`, wrapper prototypes now promoted to `src/wrappers/`, playground demos, `ai-docs/ACCESSIBILITY_CHECKLIST.md`.
...
18. **Sheet separator tokenisation**
    - Question: should the design system expose a dedicated overlay/sheet divider token (`--em-shadows-sheetDivider` or similar) to avoid hand-managed borders in `Dialog`?
    - Rationale: current implementation applies `borderLeft: borderMuted` within `sheetContentBase`; the tokenised value works, but a first-class divider token would document intent and ease future theming.
    - Notes: revisit during the design system refresh or when Storybook axe sweeps tackle overlay theming.
