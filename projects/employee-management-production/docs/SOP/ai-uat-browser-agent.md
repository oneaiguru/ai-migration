...
This SOP documents how to run Stage 6 AI UAT with ChatGPT’s browsing/agent mode. The goal is to confirm production behaviour on the latest Vercel build against the legacy reference without touching code or attempting remote Playwright runs. After every production deploy, create or refresh a timestamped prompt file under `ai-uat/` (e.g. `ai-uat/2025-10-08_stage6_agent_prompt.txt`) and paste that single message into the ChatGPT session.
