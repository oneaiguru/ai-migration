...
import { SiteAccuracyCard } from './SiteAccuracy';
...
export const Sites: React.FC = () => {
  ...
  const siteForecastId = (() => {
    ...
    return (usp.get('sf') || usp.get('siteForecast') || '').trim();
  })();
  ...
  return (
    <div className="space-y-6" data-testid="screen-sites">
      {/* Accuracy summary */}
      <SiteAccuracyCard />
      {/* Dev preview: optional site forecast via ?sf=S1 or ?siteForecast=S1 */}
      {siteForecastId && (
        <div className="card">
          <h3 className="text-lg font-semibold mb-2">Прогноз по площадке {siteForecastId}</h3>
          <SiteForecast siteId={siteForecastId} days={7} />
        </div>
      )}
      ...
      <div className="card">
        ...
        {filteredSites.length === 0 ? (
          <div className="p-6 text-sm text-gray-600 bg-gray-50 rounded border border-gray-200">
            Нет данных по площадкам на выбранную дату.
          </div>
        ) : (
