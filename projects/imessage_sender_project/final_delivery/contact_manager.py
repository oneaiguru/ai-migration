    ...
    def load_from_csv(self, file_path: str) -> int:
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as csv_file:
                ...
                # Если заголовков нет, используем стандартные
                if not headers:
                    self._log('warning', "CSV-файл не содержит заголовков, используем стандартные")
                    headers = ['phone', 'name']
                    csv_file.seek(0)  # Вернуться в начало файла
                
                # Индекс столбца с номером телефона
                phone_index = -1
                ...
                for row in reader:
                    ...
                    for i, value in enumerate(row):
                        if i != phone_index and i < len(headers):
                            contact_data[headers[i]] = value
