# Plan – Stage 6 AI UAT Comparison Report (nsp559gx9 vs gnlqewuz2)
...
- **Date:** 2025-10-08
- **Owner:** Planning agent (handoff to next execution agent)
- **Scope:** Archive the Stage 6 AI UAT comparison report, update evidence references, and surface the new asset in onboarding docs.
- **Sources:** `/Users/m/Desktop/Untitled-4.tex` (markdown-formatted report), `docs/Archive/stage-6-ai-uat/`
- **Related docs:** Stage 6 regression archive, Session handoff, TODO checklist, readlist, parity plan.

## Preconditions
```bash
set -euo pipefail
pwd  # should be ~/git/client/employee-management-parity
git status -sb  # expect a clean tree before applying
```

## Step 1 – Add Stage 6 UAT report to archive
Create a markdown file under `docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md` containing the comparison text.

```bash
cat <<'MARKDOWN' > docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md
...
* With `localStorage` flag `VITE_USE_TANSTACK_TABLE=true`, scrolling through 12k rows remains smooth (no jitter). Keyboard navigation and virtualization boundaries behave as before. ✅ **Parity**
...
## Step 2 – Reference the new report in documentation
Update the parity plan evidence inventory, readlist, handoff, and TODO checklist so future agents know where to find the report.
...
@@
   ...
   - `docs/Archive/Migration_Plan_Analysis.md` *(Stage 6 migration strategy review, archived from desktop note)*
   - `docs/Archive/stage-6-ai-uat/Stage-6-Refactor-Issues-Beyond-the-Checklist.{md,pdf}` *(Stage 6 regression sweep with embedded screenshots)*
+  - `docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md` *(Stage 6 AI UAT results comparing latest vs prior refactor builds)*
*** End Patch
PATCH
...
@@
-7. `docs/Tasks/stage-6-ai-uat-checklist.md` – browser-agent regression playbook (covers both Vercel URLs and reporting template).
+7. `docs/Tasks/stage-6-ai-uat-checklist.md` – browser-agent regression playbook (covers both Vercel URLs and reporting template).
+8. `docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md` – latest comparison results between the two refactor builds.
-8. ADRs (all short reads):
+9. ADRs (all short reads):
@@
-9. `docs/System/tech-stack-decisions.md:1-40` – cost/licence guardrails.
-10. `docs/TODO_AGENT.md:40-95` – immediate next steps (Stage 0 evidence/tests, research brief, Stage 3 flag rollout + performance note).
-11. `docs/SESSION_HANDOFF.md` (sections 2–5) – setup, evidence requirements, wrapper notes.
+10. `docs/System/tech-stack-decisions.md:1-40` – cost/licence guardrails.
+11. `docs/TODO_AGENT.md:40-95` – immediate next steps (Stage 0 evidence/tests, research brief, Stage 3 flag rollout + performance note).
+12. `docs/SESSION_HANDOFF.md` (sections 2–5) – setup, evidence requirements, wrapper notes.
-12. `docs/Archive/Migration_Plan_Analysis.md` – archived desktop strategy memo (Phase 6 rationale).
-13. Migration prep workspace:
+13. `docs/Archive/Migration_Plan_Analysis.md` – archived desktop strategy memo (Phase 6 rationale).
+14. Migration prep workspace:
@@
-14. Wrapper/code touchpoints:
+15. Wrapper/code touchpoints:
@@
-15. Production components slated for migration (skim for layout/behaviour):
+16. Production components slated for migration (skim for layout/behaviour):
@@
-16. Stage 3 reference: wrappers + demos for the TanStack table path (now gated by `VITE_USE_TANSTACK_TABLE`):
+17. Stage 3 reference: wrappers + demos for the TanStack table path (now gated by `VITE_USE_TANSTACK_TABLE`):
*** End Patch
PATCH
...
@@
-8. **Stage 6 AI UAT (browser agent, post-deploy)**: the scripted regression checklist lives in `docs/Tasks/stage-6-ai-uat-checklist.md`. Keep both deployments live (refactor `https://employee-management-parity-gnlqewuz2-granins-projects.vercel.app`, legacy `https://employee-management-parity-legacy-7b28yt9nh-granins-projects.vercel.app`) and log results in this handoff.
+8. **Stage 6 AI UAT (browser agent, post-deploy)**: the scripted regression checklist lives in `docs/Tasks/stage-6-ai-uat-checklist.md`. Keep both deployments live (refactor `https://employee-management-parity-nsp559gx9-granins-projects.vercel.app`, previous `https://employee-management-parity-gnlqewuz2-granins-projects.vercel.app`, legacy `https://employee-management-parity-legacy-7b28yt9nh-granins-projects.vercel.app`) and log results in this handoff. The latest comparison report is archived at `docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md`.
*** End Patch
PATCH

apply_patch <<'PATCH'
*** Begin Patch
*** Update File: docs/TODO_AGENT.md
@@
-- Stage 6 regression archive (`docs/Archive/stage-6-ai-uat/Stage-6-Refactor-Issues-Beyond-the-Checklist.md`) обновлён: импорт, массовая смена команды, подсветка тегов и KPI округление закрыты 2025‑10‑08; остаются только демо-placeholder'ы (фото/статусы/навыки) — документируй решение или конвертируй в задачу, если потребуется функционал.
+- Stage 6 regression archive (`docs/Archive/stage-6-ai-uat/Stage-6-Refactor-Issues-Beyond-the-Checklist.md`) обновлён: импорт, массовая смена команды, подсветка тегов и KPI округление закрыты 2025‑10‑08; остаются только демо-placeholder'ы (фото/статусы/навыки) — документируй решение или конвертируй в задачу, если потребуется функционал.
+- Stage 6 comparison report (`docs/Archive/stage-6-ai-uat/Stage-6-UAT-Report-nsp559gx9-vs-gnlqewuz2.md`) captures the latest browser-agent results; review before running additional UAT.
*** End Patch
PATCH
```

## Step 3 – Clean up source note (optional)
No destructive action required on the desktop file; retain as backup. Add a note in the handoff if physical cleanup is desired later.

## Tests
Documentation-only change; no automated tests required.

```bash
# No test commands (docs update)
```

## Acceptance Criteria
- Archived report file exists with full comparison content.
- Evidence inventory, readlist, handoff, and TODO point to the new report.
- `git status -sb` shows modified docs plus the new markdown file only.

---

**Next Plan:** _None – update `PROGRESS.md` to reflect completion._
