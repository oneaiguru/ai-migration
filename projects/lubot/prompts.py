...
def get_prompt(key: str, gender: str) -> str:
    if key not in PROMPTS:
        raise ValidationError(f"Prompt not found for key: {key}")

    prompt_value = PROMPTS[key]

    # If it's a string, return directly (gender-agnostic prompt)
    if isinstance(prompt_value, str):
        return prompt_value

    # If it's a dict, access by gender
    if isinstance(prompt_value, dict):
        if gender not in prompt_value:
            raise ValidationError(f"Prompt not found for key: {key}, gender: {gender}")
        return prompt_value[gender]

    raise ValidationError(f"Invalid prompt type for key: {key}")
