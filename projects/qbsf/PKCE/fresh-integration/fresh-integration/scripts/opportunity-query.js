...
/**
 * Query Salesforce for pricebook entries
 * @param {string} instanceUrl - Salesforce instance URL
 * @returns {Promise<Array>} - Array of pricebook entry records
 */
async function queryPricebookEntries(instanceUrl) {
  if (!instanceUrl) {
    instanceUrl = Object.keys(oauthManager.initializeTokenStorage().salesforce)[0];
  }
  ...
  try {
    ...
    // Query pricebook entries with product information
    const query = `
    ...
    console.log('Executing pricebook entry query:', query);
    const result = await conn.query(query);
    
    console.log(`Found ${result.records.length} pricebook entries.`);
    return result.records;
  } catch (error) {
    console.error('Error querying pricebook entries:', error.message);
    throw error;
  }
}
...
if (require.main === module) {
  ...
  if (command === 'opportunity' && arg) {
    // Get opportunity by ID
    console.log(`Getting opportunity by ID: ${arg}`);
    getOpportunityById(arg)
  ...
  } else if (command === 'pricebook') {
    // Get pricebook entries
    queryPricebookEntries()
      .then(entries => {
