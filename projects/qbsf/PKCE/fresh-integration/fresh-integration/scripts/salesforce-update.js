...
/**
 * Test the ability to update a Salesforce opportunity
 * @param {string} opportunityId - Opportunity ID
 * @param {string} instanceUrl - Salesforce instance URL
 * @returns {Promise<Object>} - Test result
 */
const testUpdate = async (opportunityId, instanceUrl) => {
  try {
    const conn = await createConnection(instanceUrl);
    
    // First get the current opportunity data to confirm it exists
    try {
      const opportunity = await conn.sobject('Opportunity').retrieve(opportunityId);
      console.log(`Found opportunity: ${opportunity.Name} (${opportunity.Id})`);
      
      // Create a test note
      const note = {
        ParentId: opportunityId,
        Title: 'Test Note from Integration',
        Body: `This is a test note created on ${new Date().toLocaleString()}\nVerifying the Salesforce integration functionality.`
      };
      
      const noteResult = await conn.sobject('Note').create(note);
      console.log(`Created test note with ID ${noteResult.id} for Opportunity ${opportunityId}`);
      ...
      console.error(`Opportunity with ID ${opportunityId} not found:`, oppError.message);
      
      // If opportunityId not found, query for a recent opportunity
      console.log('Querying for a recent opportunity instead...');
      ...
      if (queryResult.records.length === 0) {
        throw new Error('No opportunities found in Salesforce.');
      }
      
      const firstOpp = queryResult.records[0];
      console.log(`Found recent opportunity: ${firstOpp.Name} (${firstOpp.Id})`);
      
      return {
        success: true,
        message: 'Used a different opportunity than requested',
        opportunity: {
          id: firstOpp.Id,
          name: firstOpp.Name
        }
    ...
    console.error('Error in test update:', error.message);
    throw error;
  }
};
...
if (require.main === module) {
  ...
  if (command === 'test' && opportunityId) {
    // Test updating a specific opportunity
    console.log(`Testing Salesforce update on opportunity ${opportunityId}...`);
    testUpdate(opportunityId)
      .then(result => {
    ...
    // Test with auto-finding an opportunity
    console.log('Testing Salesforce connection by finding a recent opportunity...');
    testUpdate('test')
      .then(result => {
        console.log('Test completed successfully:');
        console.log(JSON.stringify(result, null, 2));
    ...
    console.log('Usage:');
    console.log('  node salesforce-update.js test [opportunityId]');
    console.log('  - Tests Salesforce connection and update capabilities');
  }
}