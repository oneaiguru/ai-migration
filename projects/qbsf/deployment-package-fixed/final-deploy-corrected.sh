#!/bin/bash
# final-deploy-corrected.sh - Исправленный deployment script для Salesforce-QuickBooks Integration

set -e

echo "🚀 Final Deployment: Salesforce-QuickBooks Integration Phase 2 - ИСПРАВЛЕННАЯ ВЕРСИЯ"
echo "======================================================================================"
...
# Step 2: Fix package.xml tags
log_step "2. Fixing package.xml XML structure..."
if [ -f "manifest/package.xml" ]; then
    # Replace <n> tags with <name> tags using sed
    sed -i.bak 's/<n>/<name>/g; s/<\/n>/<\/name>/g' manifest/package.xml
    log_info "✅ Package.xml structure fixed"
else
    log_error "❌ manifest/package.xml not found"
    exit 1
fi
...
if [ "$USE_NEW_CLI" = true ]; then
    log_info "Using new sf CLI commands..."
    
    # Validate first
    log_info "Validating deployment..."
    if sf project deploy validate --manifest manifest/package.xml; then
        log_info "✅ Validation successful"
        
        # Deploy for real
        log_info "Deploying metadata..."
        if sf project deploy start --manifest manifest/package.xml; then
    ...
    # Validate first
    if sfdx force:source:deploy --checkonly --manifest manifest/package.xml; then
        log_info "✅ Validation successful"
        
        # Deploy for real
        if sfdx force:source:deploy --manifest manifest/package.xml; then
            log_info "✅ Deployment successful"
        else
            log_error "❌ Deployment failed"
            exit 1
...
if [ "$USE_NEW_CLI" = true ]; then
    ...
    sfdx force:apex:test:run --testlevel RunLocalTests --outputdir test-results --resultformat human
fi
...
if [ "$USE_NEW_CLI" = true ]; then
    sf apex run --file setup-integration.apex
else
    sfdx force:apex:execute --apexcodefile setup-integration.apex
fi

# Cleanup
rm -f setup-integration.apex
rm -f manifest/package.xml.bak
