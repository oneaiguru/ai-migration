...
function createQuickBooksAuthError(code, message, qbConfig = config.quickbooks) {
  const error = new Error(message);
  error.code = code;
  error.statusCode = 401;
  error.reauthorizeUrl = getQuickBooksAuthUrl(qbConfig);
  return error;
}
...
async function refreshQuickBooksToken(realmId, qbConfig = config.quickbooks) {
  ...
  try {
    const tokens = await initializeTokenStorage();
    qbTokens = tokens.quickbooks[realmId];
    
    if (!qbTokens || !qbTokens.refreshToken) {
      throw createQuickBooksAuthError(
        'NO_TOKENS',
        `No refresh token found for QuickBooks realm ${realmId}.`,
        qbConfig
      );
    }
    ...
    if (statusCode === 400 && responseData?.error === 'invalid_grant') {
      throw createQuickBooksAuthError(
        'AUTH_EXPIRED',
        `QuickBooks refresh token is invalid or expired for realm ${realmId}. Manual reauthorization required.`,
        qbConfig
      );
    } else if (statusCode === 401) {
      throw createQuickBooksAuthError(
        'AUTH_EXPIRED',
        `Authentication failed for QuickBooks realm ${realmId}. Client credentials may be invalid.`,
        qbConfig
      );
    } else {
...
async function refreshSalesforceToken(instanceUrl, sfConfig = config.salesforce) {
  ...
  try {
    const tokens = await initializeTokenStorage();
    sfTokens = tokens.salesforce[instanceUrl];
