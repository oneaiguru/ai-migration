#!/bin/bash
# Corrected test coverage fix script for QuickBooks-Salesforce integration
# Using proper sf command syntax (2024)
...
echo ""
echo "Proper sf commands for test coverage (current syntax):"
echo ""
...
echo ""
echo "4. Check code coverage information:"
echo "sf data query --query \"SELECT ApexClassOrTrigger.Name, NumLinesCovered, NumLinesUncovered FROM ApexCodeCoverage WHERE ApexClassOrTrigger.Name IN ('QuickBooksAPIService','QuickBooksInvoiceController')\" --target-org $ORG_ALIAS"
echo ""
...
if [[ $DEPLOY_NOW == "y" || $DEPLOY_NOW == "Y" ]]; then
    ...
    sf data query --query "SELECT ApexClassOrTrigger.Name, NumLinesCovered, NumLinesUncovered, (NumLinesCovered/(NumLinesCovered+NumLinesUncovered))*100 CoveragePercentage FROM ApexCodeCoverage WHERE ApexClassOrTrigger.Name IN ('QuickBooksAPIService','QuickBooksInvoiceController')" --target-org $ORG_ALIAS
