...
function transformToInvoice(opportunityData, customerId, itemMappings = {}) {
  ...
  // Generate invoice reference number (can be customized)
  const docNumber = `SF-${opportunity.Name.replace(/[^a-zA-Z0-9]/g, '').substring(0, 15)}`;
  ...
  const invoice = {
    ...
    TxnDate: txnDate,
    Line: transformToLineItems(lineItems, itemMappings),
    CustomerMemo: {
...
// Execute if called directly with test data
