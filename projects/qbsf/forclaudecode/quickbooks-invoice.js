...
const createInvoiceFromOpportunity = async (opportunityData, realmId) => {
  try {
    ...
    const customerData = {
      ...
      PrimaryEmailAddr: account.Email__c ? { Address: account.Email__c } : undefined,
    ...
    const processedLineItems = await processLineItems(realmId, lineItems);
    
    if (processedLineItems.length === 0) {
      throw new Error('No valid line items found for the invoice');
    }
...
if (require.main === module) {
  console.log('Run this module with opportunity data to create an invoice');
}
