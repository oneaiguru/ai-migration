...
const STATE_PATH = path.join(__dirname, '../../data/oauth_state.json');
...
const saveState = (service, state) => {
  const dataDir = path.dirname(STATE_PATH);
  if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
  }
  const current = fs.existsSync(STATE_PATH)
...
const validateState = (service, stateFromRequest) => {
  if (!stateFromRequest) return false;
  try {
    const current = fs.existsSync(STATE_PATH)
      ? JSON.parse(fs.readFileSync(STATE_PATH, 'utf8'))
      : {};
    const expected = current[service];
    const isValid = Boolean(expected) && expected === stateFromRequest;
    if (isValid) {
