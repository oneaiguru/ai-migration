# Обновление файла API
cd /Users/m/git/clients/qbsf/automated-integration/src/routes/
cat > api.js << 'EOL'
/**
...
# Запустить сервер и оставить его работающим
cd /Users/m/git/clients/qbsf/automated-integration/
kill $(lsof -ti:3000) 2>/dev/null || true
node src/server.js &
sleep 3

# Проверить соединение
curl -H "X-API-Key: test_api_key_123" http://localhost:3000/health

# Команда для создания счета из выбранной сделки Salesforce (можно показать клиенту)
curl -X POST \
...
# Команда для проверки статуса оплаты (запускается вручную или по расписанию)
curl -X POST \
  -H "Content-Type: application/json" \
  -H "X-API-Key: test_api_key_123" \
