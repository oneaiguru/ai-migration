...
```mermaid
graph LR
    A[API аутентификация] --> B[75% покрытие тестами]
    B --> C[Развертывание компонентов]
    C --> D[Исправление эндпоинта API]
    D --> E[Готов к продакшену]
...
sequenceDiagram
    ...
    U->>SF: Создать Opportunity
    U->>SF: Изменить статус на "Proposal and Agreement"
    SF->>MW: HTTP POST /api/opportunity-to-invoice
    MW->>QB: Создать счет
    QB-->>MW: Invoice ID
    MW-->>SF: Обновить Opportunity.QB_Invoice_ID__c
```
...
```mermaid
graph LR
    A[Salesforce вызывает<br/>/api/create-invoice] 
    B[Middleware ожидает<br/>/api/opportunity-to-invoice]
    A -.->|❌ Неверно| B
    C[Исправлено в<br/>QuickBooksAPIService.cls]
    B --> C
    style A fill:#FFCDD2
    style B fill:#C8E6C9
    style C fill:#90EE90
...
```mermaid
graph TD
    A[Подготовка к продакшену] --> B{Есть ли учетные данные?}
    B -->|Да| C[Salesforce Connected App]
    B -->|Нет| D[❌ СТОП: Запросить у пользователя]
    C --> E[QuickBooks Production App]  
    E --> F[SSH доступ к серверу]
    F --> G[✅ Готов к развертыванию]
    D --> H[Получить недостающие данные]
    H --> B
...
- [ ] **Пароль сервера**: `3Sd5R069jvuy[3u6yj` (актуальность?)
...
```mermaid
graph LR
    A[Создать тестовую Opportunity] --> B[Изменить Stage]
    B --> C[Проверить QB_Invoice_ID__c]
    C --> D[Проверить в QuickBooks]
    D --> E[✅ Интеграция работает!]
