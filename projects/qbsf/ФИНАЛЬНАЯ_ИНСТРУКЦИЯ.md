...
```mermaid
graph LR
    A[Salesforce Opportunity] -->|Меняем этап| B["Этап: Proposal and Agreement"]
    B -->|Автоматически| C[Trigger срабатывает]
    C -->|Через 30-60 сек| D[QuickBooks счет создан]
    D --> E[Поле QB_Invoice_ID__c заполнено]
```
...
```mermaid
graph TD
    A[Opportunity] --> B{Есть Account?}
    B -->|Да| C{Есть Products?}
    B -->|Нет| X[❌ Счет не создастся]
    C -->|Да| D[✅ Счет создастся в QB]
    C -->|Нет| X
```
...
sequenceDiagram
    ...
    T->>Q: Запуск QBInvoiceIntegrationQueueable
    Q->>M: HTTP запрос к sqint.atocomm.eu
    M->>QB: Создание счета через QB API
    QB->>M: Возврат номера счета (например: 2047)
    M->>SF: Обновление QB_Invoice_ID__c
    SF->>SF: ✅ Готово! Счет создан
```
...
```mermaid
flowchart TD
    A[Создать Opportunity] --> B[Добавить Products]
    B --> C[Stage = Proposal and Agreement]
    C --> D[Ждать 60 секунд]
    D --> E{QB_Invoice_ID__c заполнено?}
    E -->|Да| F[✅ Интеграция работает!]
    E -->|Нет| G[❌ Проверить server logs]
```
