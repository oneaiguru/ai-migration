# This is an update to the existing watchdog_manager.py to include our new script
    ...
    if args.add:
        ...
        for name, path in default_scripts.items():
            if name not in manager.scripts:
                try:
                    manager.add_script(name, path)
                    logger.info(f"Added default script '{name}' with path '{path}'.")
                except FileNotFoundError as e:
                    logger.error(e)
