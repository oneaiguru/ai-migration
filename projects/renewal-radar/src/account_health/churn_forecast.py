...
def generate_churn_forecast(
    ...
    # Explicit empty account_ids list means "nobody" - return empty forecast.
    if request.account_ids is not None and len(request.account_ids) == 0:
    ...
    if not population_ids:
        # Empty population means no accounts to forecast (truly empty registry).
        # Only fall back to touchpoints if account_ids was None.
        if request.account_ids is None and not touchpoint_df.empty and "account_id" in touchpoint_df.columns:
