...
- data/raw/:
  - weather/era5land_irkutsk_2022_2024.nc
...
1) Install deps: git, curl, wget/aria2, unzip; Python 3.10+.
...
3. NOAA GHCN‑Daily (optional): scripts/agentmode/noaa_ghcn_pull.sh
4. geoBoundaries RU ADM2: scripts/agentmode/geoboundaries_pull.sh
5. RU Holidays: scripts/agentmode/holidays_generate.py
6. Fill provenance/*.json and produce reports/weather_ab/* per A/B task.

Important: Do not push raw station data or licensed files to client repos. Keep artifacts under data/raw/ and attribute sources.
