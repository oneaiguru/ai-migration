...
export const Routes: React.FC = () => {
  ...
  const [strict, setStrict] = useState<ApiRouteRec[]>([]);
  const [showcase, setShowcase] = useState<ApiRouteRec[]>([]);
    ...
    (async () => {
      ...
      try {
        const [s1, s2] = await Promise.all([
          apiGet<ApiRouteRec[]>('/api/routes', { date, policy: 'strict' }),
          apiGet<ApiRouteRec[]>('/api/routes', { date, policy: 'showcase' }),
        ]);
        let strictRecs = Array.isArray(s1) ? s1 : [];
        let showRecs = Array.isArray(s2) ? s2 : [];
        // Fallback to sites-derived policies if routes API is empty
        if ((strictRecs.length === 0 || showRecs.length === 0)) {
          const sites = await apiGet<ApiSite[]>('/api/sites', { date }).catch(() => [] as ApiSite[]);
          if (Array.isArray(sites) && sites.length) {
            if (strictRecs.length === 0) {
              strictRecs = sites
        ...
        if (!mounted) return;
        setStrict(strictRecs);
        setShowcase(showRecs);
      } catch (e: any) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="btn-primary" onClick={() => downloadCsv('strict')}>Скачать CSV</button>
          </div>
        ...
        <div className="card">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="btn-primary" onClick={() => downloadCsv('showcase')}>Скачать CSV</button>
          </div>
