...
| OS (Order Status) | 3 | Status inquiry calls |
| TS (Tire Service) | 4 | Tire service booking |
| NI (Needs Identification) | 12 | Customer qualification questions |
| PP (Product Presentation) | 7 | How products are offered |
...
## OS: ORDER STATUS CHECKPOINTS

### OS01: Order/Phone Identification Request

**Requirement (Russian):**
> "Подскажите, пожалуйста, номер заказа или телефон, который оставляли в заказе?"

**PDF Source:** Консультация по заказу / оплате заказа

**Category:** OS | **Applicability:** All status inquiry calls

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent MUST ask about order/phone:
  - Order words: "номер заказа" OR "заказ"
  - Phone words: "телефон" + "оставляли"
```

**Regex Pattern:**
```regex
(?i)(номер\s+заказа|телефон.*оставляли|заказ.*номер)
```

**Pass Examples:**
- "Подскажите, пожалуйста, номер заказа или телефон, который оставляли в заказе?"
- "Какой номер заказа или телефон?"

**Fail Examples:**
- Proceeding without asking for order/phone identification
...
**Requirement (Russian):**
> "В заказе у Вас (озвучить товар) в количестве (озвучить количество), все верно?"
...
**Detection Logic:**
```
Agent SHOULD confirm order contents:
  - Confirmation phrase: "в заказе у вас" OR "озвучить товар"
  - Verification: "все верно?" OR "верно?"
```

**Regex Pattern:**
```regex
(?i)(в\s+заказе\s+у\s+вас|озвучить\s+товар|всё?\s+верно)
```

**Pass Examples:**
- "В заказе у Вас (озвучить товар) в количестве (озвучить количество), все верно?"
- "Ваш заказ содержит X в количестве Y, правильно?"
...
**Requirement (Russian):**
> Status types: Предзаказ / Самовывоз / Доставка/ТК
...
**Detection Logic:**
```
Agent SHOULD communicate appropriate status:
  - Preorder: "заказ актуален?"
  - Pickup: "Вы можете забрать товар самовывозом после того, как Вам придет смс"
  - Delivery: "Ваш заказ еще в пути, как только заказ будет готов к выдаче, Вам придет смс"
```

**Status Types from Source:**
- Предзаказ: "заказ актуален?"
- Самовывоз: "Вы можете забрать товар самовывозом после того, как Вам придет смс"
- Доставка/ТК: "Ваш заказ еще в пути, как только заказ будет готов к выдаче, Вам придет смс"

**Pass Examples:**
- (Preorder) "Скажите, заказ все еще актуален?"
- (Pickup) "Вы можете забрать товар самовывозом после смс"
- (Delivery) "Ваш заказ еще в пути, смс придет когда готово"
...
## TS: TIRE SERVICE CHECKPOINTS

### TS01: Purchase Verification

**Requirement (Russian):**
> "Вы у нас покупали шины/диски? Или оформляли сезонное хранение?"

**PDF Source:** Шиномонтаж

**Category:** TS | **Applicability:** Tire service booking calls

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
IF customer calls about tire service, agent SHOULD verify:
  - Purchase: "покупали" + ("шины" OR "диски")
  - Storage: "сезонное хранение"
```

**Regex Pattern:**
```regex
(?i)(покупали\s+(шины|диски)|сезонное\s+хранение)
```

**Pass Examples:**
- "Вы у нас покупали шины/диски?"
- "Или оформляли сезонное хранение?"
...
**Requirement (Russian):**
> "Проверить, поступало ли смс о готовности заказа (есть запись 'Заказ в пункте самовывоза')"

**PDF Source:** Шиномонтаж

**Category:** TS | **Applicability:** Tire service booking (requires SMS confirmation)

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent MUST verify SMS received:
  - SMS confirmation: "смс" + ("готовность" OR "пункте самовывоза")
  - If no SMS: "перезвоните когда получите"
```

**Trigger:** "Можно ли мне записаться на шиномонтаж?"

**Pass Examples:**
- "Вы получили смс о том, что заказ готов?"
- "У вас должна быть смс с подтверждением"
...
**Requirement (Russian):**
> "Вы записаны на (озвучить дату и время), приезжать нужно за 10-15 минут"
> "перед посещением шиномонтажа машина должна быть пустая"

**PDF Source:** Шиномонтаж

**Category:** TS | **Applicability:** After appointment booked

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent MUST confirm appointment details:
  - Confirmation: "записаны на" + date/time
  - Early arrival: "за 10-15 минут"
  - Car prep: "машина должна быть пустая"
```

**Regex Pattern:**
```regex
(?i)(записаны\s+на|за\s+10.?15\s+минут|машина.*пустая)
```

**Pass Examples:**
- "Вы записаны на (озвучить дату и время), приезжать нужно за 10-15 минут"
- "Приезжайте за 10-15 минут до времени записи"
- "Машина должна быть пустая перед приездом"
...
**Requirement (Russian):**
> "Обслуживаем только легковые автомобили. Грузовые, мотоциклы, велосипеды и прочую технику не обслуживаем"

**PDF Source:** Шиномонтаж

**Category:** TS | **Applicability:** When customer has non-passenger vehicle

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent MUST state restrictions:
  - Passenger only: "легковые автомобили"
  - No service for: "грузовые" OR "мотоциклы" OR "велосипеды"
```

**Regex Pattern:**
```regex
(?i)(легковые\s+автомобили|грузовые|мотоциклы|велосипеды|прочую\s+технику)
```

**Pass Examples:**
- "Обслуживаем только легковые автомобили"
- "Грузовики, мотоциклы и велосипеды не обслуживаем"
...
**Requirement (Russian):**
> "Секретки — это особый вид крепежа для защиты колес от снятия"
> "Датчики давления — приборы совмещённые с вентилем, контролируют давление в шинах"

**PDF Source:** Page 17-18, Upsell explanation blocks

**Category:** PP | **Applicability:** ORDER calls with upsell offer (when PP05 triggers)

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
When offering upsells (секретки/датчики), agent SHOULD explain what they are:
  - Security bolts explanation: "защит" + ("колес" OR "от снятия" OR "крепеж")
  - Sensor explanation: "датчик" + ("давлен" OR "шин" OR "контрол" OR "вентил")
  - Either: describes function/benefit, not just product name
```

**Regex Pattern:**
```regex
(?i)(секрет[ноыкие]{0,5}.{0,40}(защит|крепеж|от\s+снятия))|(датчик[иов]{0,2}.{0,40}(давлен|вентил|контрол|шин))|(давлен.{0,40}датчик)
```

**Pass Examples:**
- "Секретки — это крепеж для защиты колес от снятия"
- "Датчики контролируют давление в шинах"
- "Это специальные болты которые защищают от кражи колес"
- "Датчики давления помогут следить за состоянием шин"

**Fail Examples:**
- "Могу предложить секретки за 1690" (no explanation of what they do)
- "Добавить датчики?" (no explanation)
- Upsell offered but product function never mentioned

**Confidence:** MEDIUM (0.80)

**Applicability:** ORDER calls only; triggered when PP05 offer is made

**Note:** PP07 is an enhancement to PP05 — both should be checked if upsell offer is made. PP05 checks the *offer*, PP07 checks the *explanation*.
...
**Requirement (Russian):**
> Branch A - "Знает размеры": "Нажать 'Подбор шин' и выставить 'По параметрам'"
> Branch B - "Не знает размеры": "Нажать 'Подбор шин' и выставить 'По автомобилю'"

**PDF Source:** Tire selection workflow

**Category:** NI | **Applicability:** All tire inquiry calls

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent asks about tire size knowledge:
  - "размеры шин" → Branch A (By parameters)
  - "марку и модель" → Branch B (By car)
```

**Regex Pattern:**
```regex
(?i)(знаете\s+размер|размеры|марку.*модель|по\s+машине)
```
...
**PDF Source:** Tire selection warnings
...
**Detection Logic:**
```
IF customer requests RunFlat tires:
  Agent MUST warn: "только в комплекте" + "нельзя" + "простыми"
```

**Regex Pattern:**
```regex
(?i)(runflat|ран\s*флет|только\s+в\s+комплекте|не\s+смешивать)
```

**Pass Examples:**
- "Шины RunFlat можно использовать только в комплекте"
...
**Requirement (Russian):**
> "на сайте все так называемые всесезонные шины помещены в лето, и они подходят только для легкой зимы - не ниже '- 5' градусов"

**PDF Source:** Tire selection special cases

**Category:** NI | **Applicability:** When customer asks about all-season tires

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
IF customer requests all-season:
  Agent MUST clarify: "легкой зимы" AND "минус 5"
```

**Regex Pattern:**
```regex
(?i)(всесезонн|легк[ао][йе]\s+зим|минус\s*5|не\s+ниже)
```

**Pass Examples:**
- "подходят только для легкой зимы - не ниже - 5 градусов"
- "для мягких зим, не ниже минус 5"
...
**Requirement (Russian):**
> "На грузовые шины, независимо от города доставки, и на заказы свыше 150 000 требуется предоплата 30%"
> "Диски МАК оформляются по предоплате не менее 20%"

**PDF Source:** Order placement prepayment rules

**Category:** PP | **Applicability:** Truck tires, orders >150k, MAC wheels
...
When presenting products:
  - Truck tires: mention "30% предоплата"
  - Orders >150k: mention "30% предоплата"
  - MAC wheels: mention "20% предоплата"
```

**Regex Pattern:**
```regex
(?i)(предоплата\s*30|предоплата.*150.?000|МАК.*20\s*%|грузовые.*30)
```

**Pass Examples:**
- "На грузовые шины требуется предоплата 30%"
- "Заказы свыше 150 000 требуют 30% предоплату"
- "Диски МАК по предоплате 20%"
...
### OH09: Wait Time Objection

**Requirement (Russian):**
> Customer: "Долго ждать / Неудобно забирать / Я работаю"
> Agent: "Я отправлю запрос менеджеру, он подскажет, что можно сделать, или отменит заказ"
> Agent: "Также могу посмотреть другие шины, которые можем привезти пораньше?"

**PDF Source:** Objection handling - Long wait

**Category:** OH | **Applicability:** Delivery time concerns
...
Customer trigger phrases:
  - "долго ждать" OR "долгая доставка"
  - "неудобно забирать" OR "неудобно приезжать"
  - "я работаю"

Agent response:
  - Offer to escalate: "запрос менеджеру"
  - Offer alternatives: "другие шины" + "пораньше"
```

**Regex Pattern:**
```regex
(?i)(долго\s+ждать|неудобно\s+забирать|я\s+работаю|привезти\s+пораньше|быстрее.*доставить)
```

**Pass Examples:**
- "Я отправлю запрос менеджеру, что можно сделать"
- "Могу посмотреть шины, которые привезим пораньше"
...
**Requirement (Russian):**
> Company warehouses: "Наличными при получении, Картой при получении, СБП (по QR-коду)"
> ТК ЛУЧ: "Наличными БЕЗ СДАЧИ"
> ТК КИТ: "Предоплата, обычно 2000 р", "частичная оплата при получении"

**PDF Source:** Payment methods by transport company

**Category:** OR | **Applicability:** Delivery orders

**Checkable From:** Transcript only ✓

**Detection Logic:**
```
Agent mentions payment options based on transport company:
  - Company warehouses: cash/card/SBP options
  - TC LUCH: "БЕЗ СДАЧИ" (no change)
  - TC KIT: prepayment requirement
```

**Regex Pattern:**
```regex
(?i)(наличными|картой|СБП|QR|предоплат|без\s+сдачи|ТК\s+(ЛУЧ|КИТ))
```

**Pass Examples:**
- "При ТК ЛУЧЬ - наличными без сдачи"
- "При ТК КИТ - требуется предоплата"
- "В наших складах - по карте, СБП или наличными"
...
| PP06 | Prepayment Warning | CONDITIONAL | Truck/MAC/High-value |
| PP07 | Upsell Explanation | RECOMMENDED | ORDER calls with upsell |
| **OBJECTION HANDLING** ||||
...
- v2.0.0 (2025-12-11): Complete rewrite from Russian PDF source
  ...
  - Added PP06 (Prepayment requirements)
  - Added PP07 (Upsell explanation - what bolts/sensors do)
  - Added OH03-OH08 (6 new objection scenarios)
  - Added OR05 (Call time preference)
  - Fixed all regex patterns for Cyrillic
  - Total checkpoints: 48 (was 22)
