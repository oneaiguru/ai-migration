...
const AdminLayout: React.FC<AdminLayoutProps> = ({ 
  ...
  const tabs = [
    ...
    { id: 'reference-data', label: 'Справочники', icon: '🗂️' },
    { id: 'ui/advanced', label: 'UI/UX', icon: '🎨' },
    { id: 'employees/enhanced-profiles', label: 'Сотрудники', icon: '👥' },
    { id: 'reports/builder', label: 'Отчеты', icon: '📊' },
    { id: 'forecasting/load-planning', label: 'Прогнозы', icon: '📈' },
    { id: 'scheduling/optimization', label: 'Оптимизация', icon: '🤖' },
    { id: 'time-attendance/dashboard', label: 'Учет времени', icon: '⏰' },
  ];
        ...
        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {currentTab === 'reports/builder' && (
                <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm">
                  ➕ Создать отчет
                </button>
              )}
              {currentTab === 'time-attendance/dashboard' && (
                <button className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md text-sm">
                  📤 Экспорт данных
                </button>
              )}
