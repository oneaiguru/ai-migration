...
## Production Supabase Credentials (CURRENT - Updated 2025-12-17)
- **Supabase URL**: `https://rlcquyttvqclcxeeghtz.supabase.co`
...
## 🚀 HANDOFF: Make Pricing Scalable for All URLs (✅ COMPLETED)

**📖 Reference**: See [HANDOFF_SCALABLE_PRICING.md](./HANDOFF_SCALABLE_PRICING.md) for full details.

**Status**: ✅ COMPLETE - All tasks done, tests passing, ready to deploy.

**What's Done**:
1. ✅ Created `config/venue_pricing.py` with Lunda rules and generic config structure
2. ✅ Refactored `get_lunda_price_for_time` → `get_venue_price_for_time` (venue-aware)
3. ✅ Updated all call sites to extract and pass `venue_id`
4. ✅ Added fallback to API price when no config/catalog available
5. ✅ **Tested Lunda**: 102 records with all 3 prices (6,000/6,250/6,500₽) ✓
6. ✅ **Tested Padel Friends**: Flat pricing uses API price correctly ✓

**Test Results**:
- **Lunda (b1280372)**: Time-based pricing working perfectly
  - 6 000 ₽ for 06:00-11:59 (Оптимальный)
  - 6 250 ₽ for 12:00-17:59 (Смешанный)
  - 6 500 ₽ for 18:00-05:59 (Прайм-тайм)
- **Padel Friends (b861100)**: Flat pricing with API fallback working
  - Uses API `price_min` for all times
  - Verified with test cases

**Key Files Modified**:
- **config/venue_pricing.py:1** - NEW venue pricing configuration (extensible for all venues)
- **src/parser/yclients_parser.py:20** - Import venue_pricing functions
- **src/parser/yclients_parser.py:55-56** - Added current_venue_id and service_catalog attributes
- **src/parser/yclients_parser.py:346** - Update call to capture_service_catalog
- **src/parser/yclients_parser.py:401** - Renamed function to capture_service_catalog
- **src/parser/yclients_parser.py:475-506** - New get_venue_price_for_time function
- **src/parser/yclients_parser.py:1180-1195** - Updated API extraction to use venue_id
- **src/parser/yclients_parser.py:1807-1820** - Updated _get_price_with_catalog_lookup
- **src/parser/yclients_parser.py:3084** - Updated select-services capture call

**How to Add New Venues**:
Just add to **config/venue_pricing.py:57**:
```python
'bVENUE_ID': {
    'name': 'Venue Name',
    'type': 'flat',  # or 'time_based'
    'price_1h': None  # for flat, or tariffs dict for time_based
}
```
